...
bboxes = struct('x',{},'y',{},'width',{},'height',{},'score',{},'cam',{}, 'class',{});
pano_bboxes = [];
...
%         disp(bbox_coords_pano);
        bbox_nms = [bbox_coords_pano(1,1) bbox_coords_pano(1,2)...
                    ...
                    cur_bboxes(i).score cur_bboxes(i).cam];
                
        pano_bboxes = [pano_bboxes; bbox_nms];
...
hold on, plot(pano_bboxes(:,1), pano_bboxes(:,2), 'r+', 'MarkerSize', 4);
% nms_bboxes = pano_bboxes;
for i = 1:length(nms_bboxes)
    box = nms_bboxes(i,1:8);
    box = reshape(box, 2,4)';
